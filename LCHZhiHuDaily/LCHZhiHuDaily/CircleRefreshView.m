...
+ (CircleRefreshView *)attachObserveToScrollView:(UIScrollView *)scrollView
    ...
    WeakObject(weakRefreshView, refreshView);
    [refreshView.activityView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(weakRefreshView);
        make.centerY.equalTo(weakRefreshView);
        make.width.equalTo(weakRefreshView);
        make.height.equalTo(weakRefreshView);
    }];
